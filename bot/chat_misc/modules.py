...
async def modules_(message: types.Message):
    ...
    kb.row(
        types.InlineKeyboardButton(
            f"⚠️ {_('settings.mute')}", callback_data="test"
        ),
        types.InlineKeyboardButton(
            f"⚠️ {_('settings.warn')}", callback_data="test"
        ),
    )
    kb.row(
        types.InlineKeyboardButton(
    ...
    kb.add(
        types.InlineKeyboardButton(
            _("settings.done"), callback_data="delete_msg"
        )
    )

    if is_inline:
        await message.edit_text(
            _("settings.modules_text"), reply_markup=kb
        )
    else:
